...
setup_window() {
    ...
    tmux send-keys -t "$session_name:$window_name.1" "cd \"$project\" && dotnet build -c Debug && dotnet run" C-m
}
...
main() {
    ...
    if ! find "$project" -maxdepth 1 -type f | grep -q "\.csproj$"; then
	echo "Invalid csharp project"
	return
    fi
